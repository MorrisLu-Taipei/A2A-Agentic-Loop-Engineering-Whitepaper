...
### （內部代號 / internal codename）ALE — TigerAI（與 arXiv:2509.06216 之 "Agentic Loop Engineering" 無關)

> ⚠️ **已被 v3.2 取代。** 本檔「內部代號 / 無關 / 棄用」的敘述已於 **v3.2 重新定位**:採「Agentic Loop Engineering (ALE)」為框架名、**不宣稱首創此名詞**(arXiv:2509.06216 已用)。現行:`ALE_WhitePaper_v3.2.md`。保留作演進證據。

---

**作者 / Author**：Yeh-Hsing Lu（Morris;虎智科技 Tiger AI Tech Co., Ltd.）
**文件類型 / Type**：White Paper · Research Framework（技術白皮書／框架研究）
**版本 / Version**：v3.0（重大更名 + 依 Codex DOI 前審做三項事實修正;見下方 Changelog）
**日期 / Date**：2026-06-21
**狀態 / Status**：Working Draft / Conceptual Framework and Research Agenda — 含初步田野觀察;受控實驗(EXP-001/002)尚未完成。

> 引用建議格式：Lu, Y.-H. (2026). *Governing Agentic Software Development with Executable Evidence: Mechanical Gates, Human Review, and Skill Capitalization* (v3.0, Working Paper). Zenodo (DOI 待鑄).
> 標題已由作者拍板(2026-06-21,採命名方案 B);內部代號 ALE 保留。

> ⚠ **重大更名(v3.0)**:原標題「**System Development Agentic Loop Engineering (ALE)**」**棄用**,因 **arXiv:2509.06216(Agentic Software Engineering Roadmap)§5.2 已正式定義 "Agentic Loop Engineering (ALE)"**(同領域、同全名、同縮寫)。為免學術引用歧義,本文改用上方新標題(**作者已拍板,命名方案 B**;以 *Mechanical Gates / Human Review / Skill Capitalization* 為關鍵詞利於檢索);「ALE」僅作 TigerAI **內部代號**,**與 arXiv:2509.06216 的 ALE 無關**(本文不宣稱 ALE 為學術命名原創)。

> **主張類型標註慣例**：核心命題標註證據等級:`[設計原則]` / `[工程規格]` / `[研究假說]` / `[初步證據]` / `[已驗證]`。對應全表見 §9.7 Claim–Evidence Matrix。截至本版,**尚無 `[已驗證]` 等級之主張**;核心命題多為 `[研究假說]` 或 `[初步證據]`,待 EXP-001/002(見 `../04_Evidence/`)產生直接資料。

> **Generative AI 使用揭露**:本文以多 AI 引擎協作生產(Claude 操作化/查核、Codex 對抗式審查、Gemini 形式化/潤稿),研究問題、田野證據、引文查核與最終責任由人類作者承擔,AI 不列為作者。完整方法論與揭露見 `../08_Methodology/`。
...
v3.0：**重大更名**(讓出與 arXiv:2509.06216 撞名的 ALE)+ 三項事實修正 + GenAI 揭露
```
...
近期實務社群將「**loop engineering**」界定為設計與維護 agent 的 plan–act–observe 迴圈,並以 guardrails 控制其行為 [Data Science Dojo 2026 等]。學術側,**[Agentic SWE Roadmap, arXiv:2509.06216] §5.2 已正式定義 "Agentic Loop Engineering (ALE)"** 為其工程活動之一。**本文因此棄用該名稱**(見標題),改以「模型共識→可執行證據」立論;本文之候選貢獻**不在迴圈本身**,而在迴圈**產出之治理與可驗證性**。
...
**可執行範例 `[工程規格]`**:下為一個對應電子發票案例(§案例研究)的 Python 不變量引擎,以 `Hypothesis` 對「通路消費張數 ≤ 全國開立總量」這條系統級上界做飽和隨機測試。其價值是:即使分頁去重的功能實作出錯,只要越過物理上界,本機械閘即 `FAIL`(對應真實 `vv_crosscheck.py` 檢查 #3 的精神):

```python
# spec_invariants_engine.py — ALE 系統級紅線不變量驗證引擎
# 規則:某通路之消費發票張數,在任何操作序列下,恆 <= 全國開立總張數。
from hypothesis import given, strategies as st

def calculate_channel_invoice_count(channel_data):
    """待測功能:計算特定通路消費發票張數。
    潛在逃逸缺陷:若分頁未穩定排序且未對唯一鍵去重,會重複累加而膨脹。"""
    total = 0
    for record in channel_data:          # 缺陷示意:未做 seen_ids 去重即累加
        total += record.get("count", 0)
    return total

class TestInvoiceInvariants:
    NATIONAL_TOTAL_LIMIT = 1_000_000     # 全國開立總量(由獨立資料集提供,非同源)
    ...
    def test_national_upper_bound_invariant(self, mock_channel_records):
        """[Spec Invariant #3] 通路消費張數恆 <= 全國開立總量。
        即使分頁排序錯誤導致重複累加,只要越過全國紅線即被攔截。"""
        computed = calculate_channel_invoice_count(mock_channel_records)
        assert computed <= self.NATIONAL_TOTAL_LIMIT, (
            f"Violation: channel count ({computed}) exceeds "
            f"national physical limit ({self.NATIONAL_TOTAL_LIMIT})."
        )
```

> **與案例的關係**:上為**教學用最小範例**(`NATIONAL_TOTAL_LIMIT` 為示意常數)。真實落地版本見專案 `06_Product/04_EInvoiceDashboard/vv_crosscheck.py`——其上界由獨立資料集 `InvoiceStatic` 即時計算,而非寫死常數;實跑結果見案例研究 v2 之真實輸出。
...
> 說明:**arXiv 條目已於 2026-06-21 經 arXiv API 逐筆核驗(編號↔題名↔發表日吻合),PDF 全文存於 `../99_References/`**。更正:`2512.03097` 實際發表日為 2025-12-01(先前 Web 檢索誤報 2024-12)。非 arXiv 文獻(書籍/標準/部落格,標 ⚠)仍須投稿前自行取得正式出處核驗(§13.6、`../99_References/README.md`)。
...
14. Anthropic (2024). *Model Context Protocol (MCP) Specification v0.1.0.* https://modelcontextprotocol.io ⚠(非 arXiv,待核)
...
16. *Agentic Software Engineering: Foundational Pillars and a Research Roadmap.* arXiv:2509.06216(核驗:2025-09-07).**§5.2 定義 "Agentic Loop Engineering (ALE)" — 本文更名之主因。**
...
22. Data Science Dojo (2026). *Agentic Loops: From ReAct to Loop Engineering (2026 Guide).* (blog) ⚠(非 arXiv,待核)
...
*— End of Working Paper (v3.0; renamed from internal codename "ALE"; Conceptual Framework + Research Agenda) —*
