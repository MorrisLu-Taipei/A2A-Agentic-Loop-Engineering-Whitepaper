...
**文件類型 / Type**：White Paper · Research Framework  |  **版本 / Version**：v3.1(精簡前置聲明;內容同 v3.0)  |  **日期 / Date**：2026-06-21
**狀態 / Status**：Working paper — 概念框架 + 研究議程;含一則初步田野觀察。受控實驗(EXP-001/002)尚未完成,故本版**無任何 `[已驗證]` 等級主張**。

> **引用建議**:Lu, Y.-H. (2026). *Governing Agentic Software Development with Executable Evidence: Mechanical Gates, Human Review, and Skill Capitalization* (v3.1, Working Paper). Zenodo (DOI 待鑄).
>
> **聲明(Disclosures)**:「ALE」僅為內部代號、非原創性宣稱;標題已更名以避開 arXiv:2509.06216 §5.2 既有的 "Agentic Loop Engineering (ALE)"。¹ 本文以多 AI 引擎協作(Claude/Codex/Gemini)生產;研究問題、田野證據與所有引文均由人類作者設定並對照原始來源查證、由其負全責——AI 不列為作者。主張等級標註(`[設計原則]`/`[工程規格]`/`[研究假說]`/`[初步證據]`/`[已驗證]`,全表見 §9.7)與完整 AI 分工見 `../08_Methodology/`。
>
