...
### （建議副標 / Recommended subtitle）An Evidence-Governed Agentic SDLC Framework for Preventing Shared-Source False Confidence in AI-produced Software

---

**作者 / Author**：Morris（虎智科技 TigerAI）
**文件類型 / Type**：White Paper · Research Framework（技術白皮書／框架研究）
**版本 / Version**：v2.5（在 v2.4 基礎上,嫁接 Gemini 的三項具體強化:需求不變量 Python 範例、成熟度數字 KPI、正式文獻;**保留 v2.4 全部治理深度**)
**日期 / Date**：2026-06-21
**狀態 / Status**：Working Draft for Academic Use — **校準版,尚未達公開出版門檻**(待 EXP-001 結果與引文全文核驗,見 §13.6)

> 引用建議格式（草案）：Morris (2026). *System Development Agentic Loop Engineering (ALE): An Evidence-Governed Agentic SDLC Framework for Preventing Shared-Source False Confidence in AI-produced Software.* TigerAI Technical White Paper, v2.5 (Working Draft).
> 註:副標題已收斂為「Evidence-Governed…」(Gemini + 投稿策略 v2 之一致建議),原副標「A Governable and Verifiable…」保留於上;**最終命名仍待 Founder 拍板**。

> **主張類型標註慣例**：為避免把設計主張誤讀為已驗證結論,本文對核心命題標註其證據等級:`[設計原則]` / `[工程規格]` / `[研究假說]` / `[初步證據]` / `[已驗證]`。對應全表見 §9.7 Claim–Evidence Matrix。截至本版,**尚無 `[已驗證]` 等級之主張**;核心命題多為 `[研究假說]` 或 `[初步證據]`,待 EXP-001(見 `../04_Evidence/ALE_EXP-001_Protocol_v2.md`)產生直接資料。
...
ALE v2.5：嫁接 Gemini 三項強化(§7.7 Python 不變量引擎、§11 數字 KPI、正式文獻);副標收斂為 Evidence-Governed
```

> **v2.4 → v2.5 改了什麼(僅嫁接,不刪治理內容)**:① §7.7 補上可執行的 Python `spec_invariants_engine.py` 範例(Hypothesis 性質測試);② §11 成熟度模型 KPI 補上**數字門檻**;③ References 以正式書目取代 ⚠ placeholder;④ 副標題收斂。**v2.4 既有之 §5.6 權限表、§7.4 三層架構、§8.5/8.6/8.7、§9.3 Figure 4、§9.6 成本工程、§9.7 Claim–Evidence Matrix、§12.2(8 條)、§13.6(7 條)全數保留**——此為相對 Gemini 精簡版(`_v2.4_gemini`)的關鍵差異:本版不丟治理深度。
...
> KPI 之數字門檻為**初始政策值 `[工程規格]`**(取自實務經驗,非實證最適值),須隨企業情境與 EXP-001/跨專案資料校準;L4/L5 之 reuse/事故率同時是 §9.7 之 C3 假說驗證資料。
...
16. *Agentic Software Engineering: Foundational Pillars and a Research Roadmap.* arXiv:2509.06216(核驗:2025-09-07).
17. *Many-to-One Adversarial Consensus: Exposing Multi-Agent Collusion Risks in AI-Based Healthcare.* arXiv:2512.03097(核驗:2025-12-01).
...
*— End of White Paper (ALE v2.5, Calibrated Working Draft) —*
