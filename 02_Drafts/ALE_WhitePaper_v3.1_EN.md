...
**Type**: White Paper · Research Framework  |  **Version**: v3.1 (condensed front matter; body identical to v3.0)  |  **Date**: 2026-06-21
...
> **Suggested citation**: Lu, Y.-H. (2026). *Governing Agentic Software Development with Executable Evidence: Mechanical Gates, Human Review, and Skill Capitalization* (v3.1, Working Paper). Zenodo (DOI to be minted).
>
> **Disclosures**: "ALE" is an internal codename only — not an originality claim; the title was changed to avoid the existing "Agentic Loop Engineering (ALE)" defined in arXiv:2509.06216 §5.2.¹ Produced with a multi-engine AI workflow (Claude / Codex / Gemini); the human author set the research questions, supplied and verified all field evidence and citations against sources, and is solely responsible — AI systems are **not** authors. Claim-level labels (`[design principle]`/`[engineering spec]`/`[research hypothesis]`/`[preliminary evidence]`/`[validated]`) and the full AI-role breakdown are defined in the companion **08_Methodology**.
>
